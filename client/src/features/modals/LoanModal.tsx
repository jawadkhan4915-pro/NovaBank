...
export const LoanModal: React.FC<LoanModalProps> = ({ isOpen, onClose, balances, activeLoans, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-md">
      <div className="w-full max-w-lg glass-hero rounded-2xl border border-glass-border p-6 shadow-2xl relative animate-in fade-in zoom-in-95">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="font-display font-bold text-lg text-ink">Crypto-Collateralized Loans</h3>
        ...
        <div className="flex bg-surface p-1 rounded-xl border border-glass-border mb-5">
          <button
            ...
            className={`w-1/2 py-2 rounded-xl font-bold text-xs transition-all ${
              tab === 'apply' ? 'bg-gold text-background shadow-gold-glow' : 'text-ink-muted hover:text-ink'
            }`}
          >
            Apply New Loan
          </button>
          <button
            ...
            className={`w-1/2 py-2 rounded-xl font-bold text-xs transition-all ${
              tab === 'repay' ? 'bg-violet text-white shadow-violet-glow' : 'text-ink-muted hover:text-ink'
            }`}
          >
            Repay Active Loan
          </button>
        ...
        {tab === 'apply' ? (
          <div className="space-y-4 mb-5">
            <div>
              <div className="flex justify-between text-xs text-ink-muted mb-1 font-mono">
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className="bg-background border border-glass-border text-ink font-mono font-bold text-xs rounded-xl px-3 py-2 focus:outline-none focus:border-gold"
                >
            ...
            <div>
              <div className="flex justify-between text-xs text-ink-muted mb-1 font-mono">
                ...
                <span className="text-gold font-bold">Max 50% LTV: ${maxAllowedUSD} USD</span>
              </div>
              <input
                type="number"
                step="100"
                ...
                className="w-full bg-background border border-glass-border rounded-xl px-3.5 py-2 text-base font-mono font-bold text-ink focus:outline-none focus:border-gold"
              />
            </div>

            {/* LTV Meter */}
            <div className="p-3.5 rounded-xl bg-surface border border-glass-border space-y-2">
              <div className="flex justify-between items-center text-xs font-semibold">
                <span className="text-ink-muted">Calculated LTV Ratio</span>
                <span className={computedLtv > 50 ? 'text-danger font-mono font-bold' : 'text-success font-mono font-bold'}>
              ...
              <div className="w-full h-2 rounded-full bg-background border border-glass-border overflow-hidden">
                <div
                  className={`h-full transition-all duration-300 ${
                    computedLtv > 50 ? 'bg-danger' : 'bg-success'
                  }`}
              ...
              {computedLtv > 50 && (
                <p className="text-xs text-danger flex items-center gap-1 font-medium">
                  <AlertCircle className="h-3.5 w-3.5" /> Requested loan exceeds 50% max LTV limit!
                </p>
              )}
            </div>
          ...
          <div className="space-y-4 mb-5">
            {activeLoans.length === 0 ? (
              <div className="p-6 text-center text-xs text-ink-faint">No active loans found to repay</div>
            ) : (
              <>
                <div>
                  <label className="text-xs font-semibold text-ink-muted block mb-1">Select Active Loan</label>
                  <select
                  ...
                  >
                    {activeLoans.map((l) => (
                      <option key={l.id} value={l.id} className="bg-background text-ink">
                ...
                <div>
                  <label className="text-xs font-semibold text-ink-muted block mb-1">Repayment USD Amount</label>
                  <input
                    type="number"
                    step="100"
                    value={repaymentUSD}
                ...
                <div className="p-3 rounded-xl bg-surface border border-glass-border text-xs flex justify-between font-mono">
                  <span className="text-ink-muted">Loan Repayment Flat Fee:</span>
                  <span className="font-bold text-gold">$1.00 USD</span>
                </div>

                <button
                  onClick={handleRepayLoan}
                  disabled={loading}
                  className="w-full py-3 rounded-xl bg-violet hover:bg-violet-dim font-bold text-xs text-white transition-all shadow-violet-glow"
                >
